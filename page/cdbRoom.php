<?php
	...
	if (isset($_POST['submit'])) {
		...
		if (mysqli_num_rows($check)>0) {
			header("Location: croom.php?error=Room already exist.");
		}else{
		$capacity = $_POST['capacity'];
		if ($capacity <=0) {
			header("Location: croom.php?invalid=put valid capacity.");
		}else{
	...
	if (isset($_POST['update'])) {
		...
		mysqli_query($db, "UPDATE db_room SET room='$room', capacity='$capacity' WHERE id=$id ");
