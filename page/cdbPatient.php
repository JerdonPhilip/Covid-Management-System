<?php
	include_once ('cdbRoom.php');
	include('cdbBarangay.php');
	//session_start();
	$name ='';
	$contact = '';
	$barangay = '';
	...
	if (isset($_POST['ptback'])) {
		...
		$id = $_POST['id'];
		mysqli_query($db, "UPDATE db_patient SET name='$name', contact='$contact', barangay='$barangay', gender='$gender', age='$age', status='$status', room='$room' WHERE id=$id ");
	...
	if (isset($_POST['discharge'])) {
		...
		$id = $_POST['id'];
		mysqli_query($db, "UPDATE db_patient SET dateofdischarge='$disc' WHERE id=$id");
		//$_SESSION['msg'] = "Information has been updated";
		header('location: cpnt.php');
	}
	//retrieve records
