<?php
require "PatientForm.php";
$db = new PDO('mysql:host=localhost;dbname=db_patient', 'root','');

class myPDF extends PatientForm{
	function header(){
	...
	function viewTable($db){
		$this->SetFont('Times','',12);
		 $stmt - $db->query('select from db_patient');
		while($data = $stmt->fetch(PDO::FETCH_OBJ)){
			...
			$this->Cell(50, 10, $data->Salary,1,0, 'L');
			$this->Ln();
		}
	}
}
